...
descriptives <- function(data, vars, param_set = "standard", by = NA, deci = 1, option = r.flex.opts, title = "", res.names) {
  ...
  res.C <- format(round(res.C, deci), nsmall = deci.q, decimal.mark = option$d.p)
  res.Q1 <- format(round(res.Q1, deci), nsmall = deci.q, decimal.mark = option$d.p)
  res.Q3 <- format(round(res.Q3, deci), nsmall = deci.q, decimal.mark = option$d.p)
